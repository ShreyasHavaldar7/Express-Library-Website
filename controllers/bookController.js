...
exports.index = function(req, res) {   
    ...
    async.parallel({
        ...
        res.render('index', { title: 'VirtuaLib Home Page', error: err, data: results });
    });
};
...
exports.book_create_post = [
    ...
    validator.body('summary', 'Summary must not be empty.').isLength({ min: 5 }).trim(),
    validator.body('isbn', 'ISBN must be 13 digits.').isLength({ min:13, max:13 }).trim(),
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                res.render('book_form', { title: 'Add Book',authors:results.authors, genres:results.genres, book: book, errors: errors.array() });
            });
            return;
        }
        else {
